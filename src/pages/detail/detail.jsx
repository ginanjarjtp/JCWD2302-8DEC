...
export default function DetailProduct(){
    ...
    let onGetData = async() => {
        try {
            let response = await axios.get(`https://my-json-server.typicode.com/ginanjarjtp/jsonserver-jcwd2302/products/${productId.id}`)
    ...
    let onAddOrder = async() => {
        try {
            ...
            let checkExist = await axios.get(`https://my-json-server.typicode.com/ginanjarjtp/jsonserver-jcwd2302/cart?idProduct=${data.id}`)

            if(checkExist.data.length === 0){
                let response = await axios.post('https://my-json-server.typicode.com/ginanjarjtp/jsonserver-jcwd2302/cart', dataToSend)
            }else{
                let newQuantity = checkExist.data[0].quantity + 1
                let update = await axios.patch(`https://my-json-server.typicode.com/ginanjarjtp/jsonserver-jcwd2302/cart/${checkExist.data[0].id}`, {quantity: newQuantity})
                console.log(update)
            }   
        } catch (error) {
        ...
        <div>
            ...
            <div className="flex mt-10">
                <div className="basis-1/3 px-24">
                    ...
                    <select ref={selectSize} onChange={onSelectSize} className="mt-3 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        {
