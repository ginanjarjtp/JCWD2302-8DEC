...
export default function Register(props){
    ...
    let onSubmit = async() => {
        try {
            ...
            let checkEmail = await axios.get(`https://my-json-server.typicode.com/ginanjarjtp/jsonserver-jcwd2302/users?email=${inputEmail}`)
            let checkUsername = await axios.get(`https://my-json-server.typicode.com/ginanjarjtp/jsonserver-jcwd2302/users?username=${inputUsername}`)
            
            if(checkEmail.data.length === 0 && checkUsername.data.length === 0){
                // Post
                let register = await axios.post('https://my-json-server.typicode.com/ginanjarjtp/jsonserver-jcwd2302/users', {username: inputUsername, email: inputEmail, password: inputPassword})
